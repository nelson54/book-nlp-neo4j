...
import novels.annotators.*;
import novels.entities.AnnotationTokens;
import novels.processors.AnnotationsProcessor;
...
public class BookNLP {
	...
	public static void main(String[] args) throws Exception {
		AnnotationTokens annotationTokens = null;
		...
		if (!tokenFile.exists() || cmd.hasOption("f")) {
			...
			annotationTokens = syntaxAnnotator.process(text);
			tokens = annotationTokens.getTokens();
		...
		if(annotationTokens != null) {
			annotationsProcessor.run(book, annotationTokens.getAnnotation(), annotationTokens.getTokens());
		}
